...
pair<int,int> MyPathList::movement(int beg, int end){
    int move = end-beg;
    if(move==1)         return make_pair(dir[1][0], dir[1][1]); // RIGHT
    else if(move==-1)   return make_pair(dir[0][0], dir[0][1]); // LEFT
    else if(move>1)     return make_pair(dir[3][0], dir[3][1]); // DOWN
    return make_pair(dir[2][0], dir[2][1]);                     // UP
}
